...
namespace DataMongoDbIntegrationTest
{
    public class MerchantControllerTest : IClassFixture<TestFixture<Startup>>
    {
        ...
        {
            var response = await _client.GetAsync($"{endpoint}/000000");
        ...
        public async Task PutEndpoint_UpdateMerchant()
        {
            ...
            var response = await _client.PutAsync("/admin/merchant/000000", ContentHelper.GetStringContent(updatedDetails));
            var getMerchant = await _client.GetAsync("/admin/merchant/000000");
